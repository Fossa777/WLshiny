            ...
            tags$div(
              ...
              tags$div(
                ...
                tags$ul(
                  ...
                  tags$li("Столбцы species, length, weight должны называться точно так же."),
                  tags$li("Строки с пустыми значениями и нечисловыми/неположительными length или weight удаляются автоматически."),
                  tags$li("Для каждого вида после фильтрации должно остаться не менее 7 наблюдений, иначе вид исключается из анализа.")
                ),
                tags$p(tags$b("Совет:"), " сначала проверьте работу на встроенном файле, затем загрузите свой Excel на вкладке 'Анализ видов'.")
              )
            )
          )
        )
